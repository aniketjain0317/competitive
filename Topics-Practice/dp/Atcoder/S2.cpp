...
intt main()
{
  ...
  int d; cin >> d;
  int n = s.size();
  vector<vector<mo>> dp(n+1,vector<mo>(d,0));
  dp[0][0]=1;
  fr(k,1,10) dp[1][k%d]+=1;
  frr(i,2,n) fr(r,0,d) fr(k,0,10) dp[i][(r+k)%d] += dp[i-1][r];
  // cum: dp[i][r] = ATMOST i digits with remainder r
  frr(i,2,n) fr(r,0,d) dp[i][r] += dp[i-1][r];
  // cnl(dp);
  function<mo(int,int)> rec = [&](int i, int cr) -> mo
  {
    if(i==n) return cr==0;
    int len = n-i-1, c = s[i]-'0', ncr = (cr+c)%d;
    mo ans = 0;
    fr(k,0,c) ans += dp[len][(d+d+d-cr-k)%d];
    return ans + rec(i+1, ncr);
  };
