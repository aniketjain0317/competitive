...
intt main()
{
  ...
  function<int(int)> rec = [&](int i) -> int
  {
    ...
    {
      // cnl("NEW:");
      // for(auto s: groups) {for(auto x: s) csp(x); cout << endl;}
      return 0;
    }
    int ch = 0, n = groups.size();
    fr(j,0,n)
    {
      int curr = 0;
      for(auto &x: groups[j]) curr += mat[i][x];
      groups[j].insert(i);
      chmax(ch, curr+rec(i+1));
      groups[j].erase(i);
    }
    groups.resize(n+1); groups[n].insert(i);
    chmax(ch, rec(i+1));
    groups.pop_back();
    return ch;
  };
