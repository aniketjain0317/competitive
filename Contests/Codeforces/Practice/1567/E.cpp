...
public:
  ...
  item merge(item& a, item& b)
  {
    ...
    ans.pref = a.pref;
    ans.suff = b.suff;
    if(tree[n+a.rx-1].num<=tree[n+b.lx].num)
    {
  ...
  item single(int v)
  {
    if(v==NEUTRAL) return {0,0,0,0,0,n,n};
    return {v,1,1,1,1,n,n};
  }
  SegTree(int sz, vector<int> arr)
  {
    ...
    for(int i = 0; i<sz; i++) tree[n+i] = single(arr[i]), tree[n+i].lx=i, tree[n+i].rx=i+1;
  ...
  {
    tree[i+=n] = single(v); tree[i].lx = i-n; tree[i].rx = i-n+1;
