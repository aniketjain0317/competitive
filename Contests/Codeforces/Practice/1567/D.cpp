...
intt main()
{
  ...
  test(t)
  {
    ...
    fr(i,0,m)
    {
      fr(j,0,s[i]-'0') st.insert(p10);
      p10*=10;
    }
    while(st.size()<n)
    {
      int x = *st.upper_bound(1);
      st.erase(st.find(x));
      fr(i,0,10) st.insert(x/10);
    }
    while(st.size()>n)
    {
      int nw = 0;
      auto it = st.begin(); nw+=*it;
      it = st.erase(it); nw+=*it;
      it = st.erase(it); st.insert(nw);
    }
    for(auto x: st) csp(x); cout << endl;
  }
}
