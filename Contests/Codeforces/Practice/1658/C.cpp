...
void solve()
{
  ...
  vvi cnt(n+1);
  vi perm(n,0);
  frrb(i,n-1,0) cnt[arr[i]].pb((n-i)%n);
  frr(i,0,n) sort(all(cnt[i]));
  // frr(i,0,n) if(!cnt[i].empty()) dsp(i), dnl(cnt[i]);
  if(cnt[1].size()!=1) cnlret("NO");
  int curr = cnt[1][0], ls = n; perm[curr] = ls--;
  int v = 1;
  while(ls)
  {
    if(cnt[v].empty()) {v++; continue;}
    int i = uppb(cnt[v], curr);
    if(i==cnt[v].size()) i=0;
    if(perm[cnt[v][i]]) {v++; continue;}
    curr = cnt[v][i];
    perm[curr] = ls--;
  }
  vi chk(n,0);
  deque<int> dq;
  fr(i,0,n) if(dq.empty()||dq.back()<perm[i]) dq.push_back(perm[i]);
  chk[0] = dq.size();
  fr(i,1,n)
  {
    int j = n-i;
    if(dq.back() == perm[j]) dq.pop_back();
    while(!dq.empty() && dq.front() < perm[j]) dq.pop_front();
    dq.push_front(perm[j]);
    chk[i] = dq.size();
  }
  // dnl(perm);
  // dnl(chk);
  fr(i,0,n)  if(chk[i]!=arr[i]) cnlret("NO");
  cnlret("YES");
}
