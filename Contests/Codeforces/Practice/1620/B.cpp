...
intt main()
{
  ...
  test(t)
  {
    ...
    {
      cin >> n[i][j];
      arr[i][j].resize(n[i][j]);
      cin >> arr[i][j];
    }
    int ans = 0;
    // height
    ans = mxs(ans, h*(maxe(arr[0][0])- mine(arr[0][0])));
    ans = mxs(ans, h*(maxe(arr[0][1])- mine(arr[0][1])));
    ans = mxs(ans, w*(maxe(arr[1][0])- mine(arr[1][0])));
    ans = mxs(ans, w*(maxe(arr[1][1])- mine(arr[1][1])));
    cnl(ans);
  }
}
