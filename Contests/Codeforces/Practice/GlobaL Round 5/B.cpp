...
intt main()
{
  ...
  vi a(n), b(n); cin >> a >> b;
  vi cnv(n+1);
  fr(i,0,n) cnv[a[i]]=i; // cnv is the mapping of given indexes to 0,1,2...n
  fr(i,0,n) b[i] = cnv[b[i]]; // apply the mapping to b
  vi ind(n,-1);
  fr(i,0,n) ind[b[i]] = i; // get indexes of each posn
