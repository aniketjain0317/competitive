...
intt main()
{
  ...
  test(t)
  {
    ...
    vector<set<int>> cnt(n+10);
    fr(i,0,n) cnt[adj[i].size()].insert(i);
    int ans = n;
    auto delEdge = [&](int u, int v)
    {
      cnt[adj[v].size()].erase(v);
      adj[v].erase(u);
      cnt[adj[v].size()].insert(v);
    };
    fr(i,0,k)
    {
      vi cnt1;
      for(auto x: cnt[1]) cnt1.pb(x);
      if(cnt1.empty()) {ans = 0; break;}
      for(auto u: cnt1)
      {
        for(auto v: adj[u]) delEdge(u, v);
        cnt[1].erase(u);
        ans--;
      }
    }
