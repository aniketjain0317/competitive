...
intt main()
{
  ...
  // test(t)
  {
    ...
    {
      int x,y; cin >> x >> y;
      int j = upper_bound(all(arr), x) - arr.begin();
      int ans = INF;
      if(j<n) chmin(ans, max(x-arr[j], 0LL) + max(y-sm+arr[j], 0LL));
      j--;
      if(j>=0) chmin(ans, max(x-arr[j], 0LL) + max(y-sm+arr[j], 0LL));
      cnl(ans);
    }
  }
}
