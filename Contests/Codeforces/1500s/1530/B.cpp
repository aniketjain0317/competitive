...
intt main()
{
  ...
  test(t)
  {
    ...
    for(int i = 0; i<w; i+=2) mat[0][i]='1';
    for(int i = 2; i<h; i+=2) mat[i][0]='1';
    for(int i = 2; i<w; i+=2) mat[h-1][i]='1';
    for(int i = 2; i<(h-1); i+=2) mat[i][w-1]='1';
    if(mat[h-2][w-1]=='1' && (mat[h-1][w-2]=='1' || mat[h-1][w-1]=='1')) mat[h-2][w-1]='0';
    for(auto s: mat) cnl(s);
  }
}
// .......(0,w-2), (0,w-1)
// .
