...
intt main()
{
  ...
  test(t)
  {
    int n,m,k; cin >> n >> k; m=n*2;
    int ch[n][2];
    fr(i,0,k) cin >> ch[i][0] >>ch[i][1];
    int used[m+1]={};
    fr(i,0,k)
    {
      used[ch[i][0]]=1;
    ...
    fr(i,0,n) if(ch[i][0]>ch[i][1]) swap(ch[i][0],ch[i][1]);
    int ans = 0;
    fr(i,0,n) fr(j,i+1,n)
    {
      if(ch[i][0] < ch[j][0] && ch[j][0] <ch[i][1] && ch[i][1]<ch[j][1]) ans++;
      if(ch[i][0] > ch[j][0] && ch[i][0] <ch[j][1] && ch[i][1]> ch[j][1]) ans++;
    }
    cnl(ans);
  }
}
