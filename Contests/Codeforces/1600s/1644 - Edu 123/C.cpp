...
void solve()
{
  ...
  vi arr(n); cin >> arr;
  vi pref(arr);
  fr(i,1,n) pref[i]+=pref[i-1];
  vi bestv(n+1, -INF); bestv[0]=0;
  frr(l,1,n)
  {
    frr(i,0,(n-l))
    {
      int j = i+l-1;
      if(!i) chmax(bestv[l], pref[j]);
      else chmax(bestv[l], pref[j] - pref[i-1]);
    }
  }
  vi bestx(bestv);
  frr(l,0,n) bestx[l]+=x*l;
  vi mxbx(bestx), mxbv(bestv);
  frr(l,1,n) chmax(mxbx[l], mxbx[l-1]);
  frrb(l,n-1,0) chmax(mxbv[l], mxbv[l+1]);
  vi ans;
  frr(l,0,n) ans.pb(max(mxbx[l], mxbv[l]+l*x));
  cnl(ans);
}
