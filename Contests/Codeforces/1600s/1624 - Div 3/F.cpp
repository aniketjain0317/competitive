...
intt main()
{
  ...
  int prev = 0, used = 1, sm = n;
  for(int l = 1024; l; l>>=1)
  {
    if(l>=n) continue;
    int ch = used?(n-l):l;
    int next = query(ch); sm+=ch;
    used = prev==next; prev = next;
    if(used) sm-=l;
  }
  sm--;
  csp("!"); cnl(sm);
}
// check for 512, ans is smaller : x+512>=n
// check for (n-512+256), ans is smaller : x+n+256>=2*n
// check for (n-256+128), ans is greater : x+n+n+128<3*n
// check for 64, ans is greater: x+n+n+128+64<3*n
// check for 32, ans is smaller, x+n+n+128+64+32>=3*n
// check for n-32+16
