...
intt main()
{
  ...
  test(t)
  {
    ...
    string s; cin >> s;
    vi st[2];
    fr(i,0,n) st[s[i]-'0'].pb(i);
    sort(all(st[0]), [&](int i, int j) {return p[i]<p[j];});
    sort(all(st[1]), [&](int i, int j) {return p[i]<p[j];});
    vi q(n);
    fr(i,0,st[0].size()) q[st[0][i]] = i+1;
    fr(i,0,st[1].size()) q[st[1][i]] = st[0].size()+i+1;
    cnl(q);
  }
}
