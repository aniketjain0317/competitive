...
intt main()
{
  ...
  {
    pi curr = q.top(); q.pop();
    if(cost[curr.sn]<=curr.fs) continue;
    cost[curr.sn] = curr.fs;
    frr(x,1,curr.sn)
    {
      int y = curr.sn+(curr.sn/x);
      if(y>1000 || cost[y]<=(curr.fs+1)) continue;
      q.push({curr.fs+1, y});
    }
  }
  test(t)
  {
    int n,k; cin >> n >> k;
    vi b(n), c(n); cin >> b >> c;
    vi wt(n); fr(i,0,n) wt[i] = cost[b[i]];
    if(sum(wt)<k) {cnl(sum(c)); continue;}
    if(k==0)
    {
      int ans = 0;
      fr(i,0,n) if(wt[i]==0) ans+=c[i];
      cnl(ans);
      continue;
    }
    vi dp(k+1, 0);
    ...
    {
      vi nw(dp);
      frr(j,0,k) if(j>=wt[i]) chmax(nw[j], dp[j-wt[i]]+c[i]);
      dp = nw;
    }
    cnl(maxe(dp));
  }
