...
void solve()
{
  ...
  // cnl(bsb.to_string());
  int ans = b-a;
  fr(x,a,b) if(chmin(ans, x-a+ (x|b)-b + 1)) ;
  fr(x,b,b+a+5) if(chmin(ans,(x|a) - b + 1)) ;
  // fr(x,a,b)
  // {
  //   bitset<32> bsx(x), bsb(b);
  //   int j = -1;
  //   frrb(i,31,0)
  //   {
  //     if(bsx[i]==1 && bsb[i]==0)  {j=i; break;}
  //   }
  //   if(j==-1) if(chmin(ans,x-a + 1)) csp(2), dnl(ans);
  //   else
  //   {
  //     int xx = x%(1<<(j+1)), bb = b%(1<<(j+1));
  //     dsp(x); dsp(b); dsp(xx); dnl(bb);
  //     if(chmin(ans, x-a + bb-xx + 1)) csp(3),dnl(ans);
  //   }
  // }
  // csp("AAAAA");
  cnl(ans);
}
// for each x: a to b-1: find min y>b s.t. a|b = b;
// but a = a|b, b;
// (a+x)|b = b+y+z
// minimize x+y+z
