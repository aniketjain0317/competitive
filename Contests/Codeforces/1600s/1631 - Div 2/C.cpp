...
intt main()
{
  ...
  {
    int n,k; cin >> n >> k;
    if(n==4 && k==3) {cnl(-1); continue;}
    map<int,int> mp;
    if(k==(n-1))
    {
      mp[k] = k-1; mp[k-1] = k;
      mp[1] = 0; mp[0] = 1;
      mp[3] = 2^(n-1); mp[2^(n-1)] = 3;
      mp[2] = 3^(n-1); mp[3^(n-1)] = 2;
    }
    else if(k!=0)
    {
      mp[k] = (n-1); mp[n-1]=k;
      mp[k^(n-1)]=0; mp[0]=k^(n-1);
    }
    vpi ans;
    int used[n]={};
    fr(i,0,n) if(!used[i])
    {
      used[i]=1;
      if(mp.find(i)!=mp.end() && i<mp[i]) ans.pb({i, mp[i]}), used[mp[i]]=1;
      else ans.pb({i, i^(n-1)}), used[i^(n-1)]=1;
    }
    cnl(ans);
  }
}
