...
intt main()
{
  ...
  // test(t)
  {
    ...
    vi con(n,-1);
    vvi id(n+1);
    fr(i,0,n) id[arr[i]].pb(i);
    frr(i,0,n) if(id[i].size()>=2)
    {
      con[id[i].front()] = id[i].back();
      con[id[i].back()] = id[i].front();
    }
    int curr = -1;
    int ans = 0, cmx = -1;
    fr(i,0,n)
    {
      if(curr==-1)
      {
        if(con[i]!=-1 && i<con[i]) curr = con[i];
      }
      else if(i==curr)
      {
        if(cmx==-1 || cmx<curr) curr ==1, ans++;
        else curr = cmx;
        cmx = -1;
      }
      else chmax(cmx, con[i]), ans++;
    }
    cnl(ans);
  }
}
