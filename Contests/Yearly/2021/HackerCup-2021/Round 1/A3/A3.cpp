...
intt main()
{
  ...
  test(t)
  {
    ...
    string s; cin >> s;
    char fs='F', ls = 'F';
    mo fsi = -1, lsi = -1, curr = 0, prefs = 0, suffs = 0, ans = 0, len = 0;
    fr(i,0,n)
    {
      char c = s[i];
      if(fs=='F')
      ...
      {
        // dbs(i); dbs(ls); dbs(lsi); dbs(curr); dbs(prefs); dbs(suffs); dbnl(ans);
        ans*=2;
        ans+=len*(prefs+suffs);
        prefs+=prefs+len*curr;
        suffs+=suffs+len*curr;
        if(fs!=ls)
        {
          ans+=(len-fsi)*(lsi+1);
          suffs+=lsi+1;
