...
intt main()
{
  ...
  test(t)
  {
    ...
    fr(i,0,n)
    {
      ...
      fr(j,0,n)
      {
        if(mat[i][j]==0) curr++, jx = j;
        else if(mat[i][j]==2) {curr=INF; break;}
      }

      if(ans>=1 && curr==1) used[jx]=1;
      if(ans==curr) cnt++;
      else if(chmin(ans,curr)) cnt=1;
    }
    fr(j,0,n) if(!used[j])
    {
      int curr = 0;
    ...
    cout << "Case #" << tno << ": ";
    if(ans>=INF) cnl("Impossible");
    else csp(ans), cnl(cnt);
  }
}
