  ...
  test(t)
  {
    ...
    for(auto &[d, vv]: dates)
    {
      for(auto &[ch, h]: vv)
      {
        ...
        {
          if(!s2.empty() && s2.find(h)!=s2.end()) s2.erase(s2.find(h));
          else
          {
            s1.erase(s1.find(h));
            sm-=h;
...
// set1: k elements
// set2: remaining elements
// add an element: insert into set1, add val to sum, push smallest_element into set2, delete smallest_element
// remove an element: remove from set1: push top set2 into set1, change sum as needed.
