...
intt main()
{
  ...
  test(t)
  {
    ...
    {
      vi s;
      fr(k,0,n) if((1<<k)&x) s.push_back(k);
      for(auto &k: s) sm[x]+=prop[k][2];
      int ch = s[0];
      for(auto &i: s)
        if(conn[x^(1<<i)])
          for(auto &j: s)
      ...
      fr(i,0,n)
      {
        if(bs(all(s), i)) continue;
        if(prop[i][0]>sm[x] || sm[x]>prop[i][1]) continue;
        int f = 0;
        for(auto &k: s) if(bs(all(adj[k]), i)) f=1;
        if(!f) continue;
        nbs[x].push_back(i);
      }
    }
    ...
    fr(x,1,nn)
    {
      ...
      {
        int mx = x^(1<<k);
        if(bs(all(nbs[mx]), k)) dp[x]+=dp[mx];
      }
      bitset<16> bts(x);
      // if(sm[x]==K) csp(bts.to_string()), cnl(dp[x]);
      if(sm[x]==K) ans+=dp[x];
    }
    tc(tno); cnl(ans);
  }
...
// dp[m] += dp[m^j] if(m^j is connected and m^j has j as a viable neighbour)
