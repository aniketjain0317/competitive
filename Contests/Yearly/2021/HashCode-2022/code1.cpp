...
public:
    ...
    int score;
    int best_before;
    vector<pair<string,pair<int,int>>> skill_level_req;
...
intt main()
{
  ...
  for(int i = 0; i<P; i++)
  {
    ...
    for(int j = 0; j<r; j++)
    {
      string skill; cin >> skill;
      int level; cin >> level;
      projects[i].skill_level_req.pb({{level, j}, string});
    }
    sort(all(projects[i].skill_level_req), greater<pair<pi, string>>)
  }

  vector<Project> sorted(projects);
  for(auto &[s, v]: skilled) sort(all(v));
  sort(all(sorted), [](auto &a, auto &b) {
    if(a.score==b.score) return a.best_before<b.best_before;
    return a.score>b.score;
  });

  auto checkMentor = [&](Project& project, vi &ids)
  {
    map<string, int> mp;
    int r = project.skill_level_req.size();
    fr(i,0,r)
    {
      auto &[skill, req] = project.skill_level_req[i];
      int &level = people[ids[i]].skill_level[skill];
      if(level<=req) level++;
    }
  };
  ...
  for(auto &project: sorted)
  {
    ...
    // if((mx+project.days)>=(project.best_before+project.score)) f=1;
    if(f) continue;
    mx+=project.days;
    for(auto &id: ids) people[id].available=mx;
    ans[project.id] = ids;
    checkMentor(project, ids);
    int score = project.score;
    if(mx>=project.best_before) score-=(mx-project.best_before);
    val+=max(0ll, score);
  }
  cnl(val);
  cnl(ans.size());
