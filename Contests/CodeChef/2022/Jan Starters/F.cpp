...
intt main()
{
  ...
  test(t)
  {
    ...
    vi ls(N,n), nx(N,n);
    vvi from(N), to(N);
    fr(i,0,q) from[ps[i].second].push_back(ps[i].first);
    fr(i,0,q) if(from[ps[i].second].size()>RQ) to[ps[i].first].pb(ps[i].second);
    ...
    {
      int c = 0;
      if(from[arr[i]].size()<=RQ)
      {
        for(auto x: from[arr[i]]) if(chmax(c, dp[ls[x]])) prev[i] = ls[x];
      }
      else
      {
        c = dp[nx[arr[i]]];
        prev[i] = nx[arr[i]];
        if(prev[i]==n) prev[i]=-1;
      }
      dp[i] = c + 1;
      ls[arr[i]] = i;

      for(auto x: to[arr[i]]) if(dp[nx[x]]<dp[i]) nx[x] = i;
    }
    ...
    reverse(all(seq));
    cnl(seq.size()); cnl(seq);
  }
}
// vi ls(n)
// vvi from(n)
//
// dp[i] = (max(arg j) dp[ls[from[arr[i]][j]]]) + 1;
// ls[arr[i]] = i;
