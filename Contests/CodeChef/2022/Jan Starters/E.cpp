...
intt main()
{
  ...
  test(t)
  {
    ...
    vi visited(n,0);
    function<void(int)> rootTree = [&](int node)
    {
      visited[node]=1;
    ...
    int flag = 0;
    vi red(n,0), green(n,0);
    function<void(int,int,int)> dfs = [&](int node, int cRed, int cGreen)
    {
      if(s[node]=='B')
      {
        if(red[cRed] || green[cGreen]) {flag = 1; return;}
        else red[cRed] = green[cGreen] = 1;
      }
      for(auto child: tree[node])
      {
        if(s[node]=='R') dfs(child, child, cGreen);
        else if(s[node]=='G') dfs(child, cRed, child);
        else dfs(child, cRed, cGreen);
      }
    };
    dfs(0,0,0);
    if(flag) cnl("NO");
